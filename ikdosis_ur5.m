...
indicador = 0; % indicador de posicion. 
...
zoffset = 1.9; % Valor en mm.
zoffset = zoffset * Factor; % Valor en m. 

% Offset del robot. 
Hoffset = 374.28; % en mm  En la mesa para el extrusor 342, para el marcador 320, con el vidrio 374.28
Hoffset = (Hoffset * Factor); % m. 
...
interpolacion = 50; %interpolaion inicial
interpolacion_media = 100; % interpolacion de puntos medios

%% Extrasion del codigo G -> Vamos a hacer una prueba con Griffin.  

% Inclusion de la libreria en el Matlab. 

fid = fopen('Configuraciones Extras/Configuraciones_rapidas/Cubo_sin_tapas_500_500_Cuadrante4_30_30.gcode', 'r', 's'); % Extraigo el archivo de codigo g 
tline = fgets(fid); % Excluye el caracter de nueva linea
...
% Determinacion de la cinematica a utilizar. 
Xmax = max(abs(matriz_codigo_g(1:end,1)));
Ymax = max(abs(matriz_codigo_g(1:end,2)));
Zmax = max(abs(matriz_codigo_g(1:end,3)));

d = norm([Xmax, Ymax, Zmax]);

% Comparamos con el indice de destreza del robot. 
if d >= (350*Factor)
    % COnfiguracion 2 de la cinematica inversa. 
    indice = 1;
else 
    % Configuraicon 1 de la cinematica inversa. 
    indice = 0;
end 
...
for i = 1:(numRows-1)
    ...
    distancia = norm(matriz_codigo_g(i,1:3) - matriz_codigo_g(i+1,1:3)); % Miro la distancia entre los datos. 
    ...
    if distancia > Hueco
        ...
        % Vamos a mirar la distancia
        P1 = T1(1:3,4);
        P2 = T2(1:3,4);
        

        % Espaciado lineal de los datos. -> Contiene inicio y fin
        x = linspace(P1(1),P2(1),interpolacion_media);
        y = linspace(P1(2),P2(2),interpolacion_media);
        z = linspace(P1(3),P2(3),interpolacion_media);

        % construccion de matrices 
        for j = 1:interpolacion_media
            tg(:,:,j) = [[1 0 0; 0 -1 0; 0 0 -1] [x(j);y(j);z(j)];[0 0 0] 1]; % Construyo las matrices de interpolacion.
        end
        ...
        for j = 1:interpolacion_media
            ...
            matriz_puntos_intermedios(:,:,indicador+j) = tg(:,:,j); % Almaceno las matrices de transformacion. 
        end

        indicador = indicador + interpolacion_media; % Sumo al valor de indicador. 
...
% 2. Antes de eso, debe llegar desde la posicion de Home hasta la posicion de inicio de trabajo. 
...
% Si se quiere alcanzar una orientacion del efector perpenticular al plano se debe especificar un Tprint
% -0.5 <= x <= 0.5 ;
% -0.5 <= y <= 0.5 ;
% -0.1 <= z <= 0.4 ;
...
tg = analitica(T,indice); % Interpolacion con los joints del robot. 
q = jtraj(qimpresion,tg,interpolacion);

for j = 1:interpolacion-1
...
for i= 1:numRows
    ...
    if matriz_codigo_g(i,4) == 0
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [Theta] = analitica(T,indice);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        % Organizando la matriz, tenemos que 
        matriz(inicio + 1,1:6) = Theta;
        ...
        for j= 1:interpolacion_media-1
            ...
            [Theta] = analitica(matriz_puntos_intermedios(:,:,iniciom+j),indice);

            % Miramos el tamaño de la matriz
            [inicio, ~] = size(matriz); % Que siempre agarre 0el valor del ultimo lugar de la matriz.
...
tg = analitica(T,indice); % Interpolacion con los joints del robot. 
q = jtraj(tg,qimpresion,interpolacion);
...
for i = rows-(interpolacion+1):rows-1
...
% convierto de m a mm 
distancia(1:end,1) = distancia(1:end,1) * (1/Factor) * 100; 
...
% desde 1.
fprintf(fid, formatSpec, matriz(1,:)); % Valor default es 1
